// 台灣縣市與鄉鎮市區。22 縣市 / 368 鄉鎮市區。
//
// 刻意內建而不是打反向地理編碼 API：那要把工地座標送給第三方服務，
// 而座標是最能精確識別案場的東西，跟這個 App 其他地方的設計互相矛盾。
// 內建清單離線可用、零外流、選起來也比等網路快。
//
// 用「臺」不用「台」，跟公文與契約書一致。

export const TW_ZONES = {
  ...
  苗栗縣: ['苗栗市', '頭份市', '竹南鎮', '後龍鎮', '通霄鎮', '苑裡鎮', '銅鑼鄉', '三義鄉', '西湖鄉', '造橋鄉', '頭屋鄉', '公館鄉', '大湖鄉', '卓蘭鎮', '三灣鄉', '南庄鄉', '獅潭鄉', '泰安鄉'],
...
export const COUNTIES = Object.keys(TW_ZONES);

export function districtsOf(county) {
  return TW_ZONES[county] || [];
}

/**
 * 組出一行可讀的工址。
 * @param {{county?: string, district?: string, village?: string, address?: string}} site
 */
export function formatSite(site) {
  if (!site) return '';
  return [site.county, site.district, site.village, site.address]
    .map((s) => (s || '').trim())
    .filter(Boolean)
    .join('');
}
